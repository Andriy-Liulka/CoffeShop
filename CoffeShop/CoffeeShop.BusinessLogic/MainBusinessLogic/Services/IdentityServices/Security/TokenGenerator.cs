...
public class TokenGenerator
{
    ...
    internal TokenDto JwtAccessToken(UserClaimDto model)
    {
        var authSigningKey =
            new SymmetricSecurityKey(
                Encoding.UTF8.GetBytes(_configuration["JWT:Secret"] ?? throw new NullReferenceException()));
        _ = int.TryParse(_configuration["JWT:AccessTokenValidityInMinutes"], out var tokenValidInMinutes);
        return new TokenDto
        {
    ...
    internal UserClaimDto DecryptAccessToken(string token)
    {
        ...
        {
            Role = principal?.Claims?.FirstOrDefault(x => x.Type.Equals(ClaimTypes.Role))?.Value ??
                   throw new NullReferenceException(),
            Name = principal?.Claims?.FirstOrDefault(x => x.Type.Equals(ClaimTypes.Name))?.Value ??
                   throw new NullReferenceException(),
            Email = principal?.Claims?.FirstOrDefault(x => x.Type.Equals(ClaimTypes.Email))?.Value ??
                    throw new NullReferenceException()
        };
    }
}